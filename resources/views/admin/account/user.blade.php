...
@section('content')
    <main class="p-6 bg-gray-50 min-h-screen flex-1" x-data="{ openAdd: false, openEdit: false, editData: {} }">
        ...
        <div class="flex justify-between items-center mb-6 user-header-wrapper">
                ...
                <table class="w-full border-collapse text-left">
                    ...
                    <tbody class="text-gray-700">
                        @forelse ($users as $row)
                            <tr
                                class="@if ($loop->even) bg-gray-50 @else bg-white @endif border-b border-gray-100 hover:bg-blue-50 transition-colors">
                                <td class="p-4 font-semibold text-gray-700">{{ $loop->iteration }}</td>
                                <td class="p-4">{{ $row['id'] }}</td>
                                <td class="p-4">
                                    {{ $row->lokawisata->nama_lokawisata ?? '-' }}
                                </td>
                                ...
                                <td class="p-4 text-center flex justify-center gap-2">
                                    <button @click="openEdit = true; editData = {{ json_encode($row) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center p-6 text-gray-500">Belum ada data</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="openAdd" x-cloak x-transition.opacity
            ...
            <div @click.away="openAdd = false" class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <form action="{{ route('user.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Pilih Lokawisata yang Dikelola</label>
                            <select name="lokawisata_id"
                                class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-400 outline-none"
                                required>
                                <option value="">-- Pilih Lokawisata --</option>
                                @foreach ($lokawisatas as $lokawisata)
                                    <option value="{{ $lokawisata->id }}">{{ $lokawisata->nama_lokawisata }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
        ...
        <div x-show="openEdit" x-cloak x-transition.opacity
            class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
            <div @click.away="openEdit = false" class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                <h2 class="text-xl font-semibold mb-4 text-gray-800">Edit Pengguna</h2>
                <form method="POST" :action="'/admin/accounts/user/' + editData.id">
                    @csrf
                    @method('PUT')
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="lokawisata_id"
                                ...
                                required>
                                <option value="">-- Pilih Lokawisata --</option>
                                @foreach ($lokawisatas as $index => $lokawisata)
                                    <option value="{{ $lokawisata->id }}" {{ $index === 0 ? 'selected' : '' }}>
                                        {{ $lokawisata->nama_lokawisata }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-500 transition">Simpan
                            Perubahan</button>
                    </div>
                </form>
            </div>
