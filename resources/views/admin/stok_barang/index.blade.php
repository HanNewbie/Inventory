...
@section('content')
    <main class="p-6 bg-gray-50 min-h-screen flex-1" x-data="{ openAdd: false, openEdit: false, editData: {} }">
        ...
        <div class="bg-white shadow-lg rounded-2xl overflow-hidden">
            ...
            @endphp
            <div
                class="px-6 py-4 bg-[#F8FEFE] border-b border-[#A1E3F9]
                ...
                <form action="{{ route('stok_barang.index') }}" method="GET"
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari barang..."
                        class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-full sm:w-64">
                    ...
                    <a href="{{ route('stok_barang.index') }}"
                        class="bg-blue-100 text-gray-800 rounded-lg px-4 py-1.5 text-sm hover:bg-blue-200 text-center">
                        ♻️ Reset
                    </a>
                </form>
            </div>
                                ...
                                <td class="p-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <form id="delete-form-{{ $st->id }}" method="POST"
                                            action="{{ route('stok_barang.destroy', $st->id) }}" class="flex">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" data-id="{{ $st->id }}"
                                                class="
                                                    delete-button
                                                    h-9 md:h-8
                                                    px-2 md:px-3
